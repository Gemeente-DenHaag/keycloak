<#macro loginCard provider providerData>
    ...
    <#if provider.displayName??>
        <#list providerData?keys as key>
            <#if provider.displayName?lower_case?contains(key)>
                ...
                <#assign imageSrc = providerData[key]>
            </#if>
        </#list>
    </#if>
    ...
    <section class="denhaag-card-authentication">
        <header class="denhaag-card-authentication__header">
            ...
            <h3 class="utrecht-heading-3 denhaag-card-authentication__title">Inloggen met ${provider.displayName!}</h3>
            <p class="utrecht-paragraph denhaag-card-authentication__subtitle"></p>
            </hgroup>
            <#if imageSrc != "">
                <p class="denhaag-image denhaag-card-authentication__logo"><img width="80" height="80" src="${imageSrc}" class="denhaag-image__image" alt="DigiD logo" loading="lazy"></p>
            </#if>
        </header>
        <div class="denhaag-card-authentication__cta">
            <a class="denhaag-card-authentication__cta-link denhaag-button denhaag-button--large denhaag-button--primary-action denhaag-button--end-icon" href="${provider.loginUrl}" target="_self" title="Inloggen met ${provider.displayName!}">
            Inloggen
        ...
        <footer class="denhaag-card-authentication__footer">
            ...
            Geen DigiD? 
            <a href="#example-link" class="denhaag-link denhaag-link--with-icon denhaag-link--with-icon-end denhaag-link--external">
